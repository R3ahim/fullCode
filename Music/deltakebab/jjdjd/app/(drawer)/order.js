...
console.log('btn',mainData)



const hanldeAlert = (id)=>{
  Alert.alert('alert')
}
            ...
            <tr class="heading" style="background:#eee;border-bottom:1px solid #ddd;font-weight:bold;">
                ...
                ${SelectedIdData.items.map(
                  ...
                  <td>
                      ...
                      <br>
 ${ 
                      result.map((item,i) => item.extraSauce.map((sauce) => `<small>${i + 1}= ${sauce.name} * ${sauce.quanity} </small>`))
                 }
                      <br>
                      
                ${ 
                      result.map((item,i) => item.extra.map((extra) => `<small>${i + 1}= ${extra.name} *</small>`))
                 }
...
<TouchableOpacity style={styles.button}  onPress={()=>setBtnData('Food Processing')}>
      <Text style={styles.text}>Pending and Accepted</Text>
    </TouchableOpacity>
<TouchableOpacity style={styles.button}  onPress={()=>setBtnData('Deliverd')}>
      <Text style={styles.text}>Deliverd</Text>
    </TouchableOpacity>
