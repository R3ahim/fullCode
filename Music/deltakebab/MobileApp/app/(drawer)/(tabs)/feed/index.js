...
const orderse = orderser.filter(order=>order.status === 'Not Accepted')
    ...
    const handleDelete = async(orderId) =>{
      ...
      const response1 = await axios.post(url+"/api/order/delete",{
        orderId
      })
      hanldeModaleOff(orderId)
        ...
        <table cellpadding="0" cellspacing="0">
            ...
            <tr class="heading" style="background:#eee;border-bottom:1px solid #ddd;font-weight:bold;">
                ...
                ${SelectedIdData.items.map(
                  ...
                  <td>
                      <b> ${item.sizeId}  ${item.name} X ${item.quantity} + ${item.sauceId}</b>
                      <br>
                      ...
                      <br>
                       ...
                       ${ 
                      result.map((item,i) => item.extraSauce.map((sauce) => `<small>${sauce.name} * ${sauce.quanity} </small>`))
                 }
                      <br>
                      
                ${ 
                      result.map((item,i) => item.extra.map((extra) => `<small> ${extra.name}  * ${extra.quanity} *</small>`))
                 }
...
<Text> {element.item.items.map((item, index) => {
          ...
          <Text>comment: {element.item.address.comment} </Text>
...
<View style={styles.modalInfo}> 
   ...
   <View>
     ...
     <Text style={{fontSize:16}}> {index} {element.item.sizeId}  {" "}  {element.item.name} X {element.item.quantity} + {element.item.meatId} + {element.item.sauceId}</Text>
     {element.item.extraSauce?.filter((sauce) => sauce.quanity > 0).length > 0 ? (
               element.item.extraSauce
                 .filter((sauce) => sauce.quanity > 0)
                 .map((sauce, index) => (
                   <Text key={index}>
                     Sauce: {sauce.name}, Price: {sauce.price}, quantity:{sauce.quanity}
                   </Text>
                 ))
             ) : (
               <p>No extra sauces selected.</p>
             )}
     {element.item.extra?.filter((sauce) => sauce.quanity > 0).length > 0 ? (
               element.item.extra
                 .filter((sauce) => sauce.quanity > 0)
                 .map((sauce, index) => (
                   <Text key={index}>
                     Extras: {sauce.name}, Price: {sauce.price} , quantity:{sauce.quanity}
                   </Text>
                 ))
             ) : (
               <p>No Seleteced Extras.</p>
             )}
      ...
      <Text>Commnet: {SelectedIdData.comment}      </Text>
      </View>
...
<TouchableOpacity style={styles.modalBtn}  onPress={()=>hanldeModaleOff(SelectedIdData._id,SelectedIdData.address.email,selectedTime,SelectedIdData.address?.firstName)}>
  ...
  <TouchableOpacity style={styles.modalBtn} onPress={()=>handleDelete(SelectedIdData._id)}  >
  <Text style={{color:'white',fontSize:20}} >Delete</Text>
</TouchableOpacity>
  <TouchableOpacity style={styles.modalBtn}  >
  <Text style={{color:'white',fontSize:20}} onPress={()=>createAndPrintSmallPDF(SelectedIdData._id)}>print</Text>
</TouchableOpacity>
